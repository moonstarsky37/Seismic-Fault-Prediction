...
# We significantly reduce the number of layers and features at each 
# layer to save GPU memory and computation but still preserve high 
# performance in fault segmentation.
...
def unet(pretrained_weights=None, input_size=(None, None, None, 1)):
    """
    Builds a simplified 3D U-Net model for fault segmentation.
    Args:
        pretrained_weights (str, optional): Path to the pre-trained weights file. Defaults to None.
        input_size (tuple): Shape of the input data (depth, height, width, channels).
    Returns:
        Model: A Keras model object of the simplified U-Net.
    """
    # Input layer
...
def cross_entropy_balanced(y_true, y_pred):
    """
    Computes balanced cross entropy loss to handle class imbalance.
    Args:
        y_true (tensor): Ground truth binary mask.
        y_pred (tensor): Predicted probabilities.
    Returns:
        tensor: Balanced cross entropy loss.
    """
    _epsilon = K.epsilon()
...
def _to_tensor(x, dtype):
    """
    Converts a Python object to a tensor of the specified data type.
    Args:
        x: Input object (numpy array, list, tensor).
